...
const getFullName = (Value) => {
  ...
  const fetchEnrollmentInfo = async (enrollment_id) => {
    try {
      const response = await fetch(
        `https://cvsu-backend-system.vercel.app/api/getEnrollmentInfo?enrollment_id=${enrollment_id}`
      );
      if (!response.ok) {
  ...
  try {
    ...
    // Optionally display an error message or handle the error as needed
        ...
        const fetchApplications = async () => {
          try {
            const response = await fetch('https://cvsu-backend-system.vercel.app/api/manage-application');
            if (!response.ok) {
  ...
  const handleSave = async () => {
    try {
      const response = await fetch(
        `https://cvsu-backend-system.vercel.app/api/applicant_progress/${editingId}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const progressUrl = `https://cvsu-backend-system.vercel.app/api/applicant-progress/${formData.enrollment_id}`; // API endpoint to update applicant progress
